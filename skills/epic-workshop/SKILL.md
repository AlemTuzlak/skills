...
- **Prefer many small 💰 over one big block — each at its final code location.** Don't pile every change into a single numbered 💰 spec at the top of the file (or in the README). That forces the learner to hold the whole plan in their head and hunt for each insertion point. Split it into one focused 💰 per change, placed *exactly where that code goes* — spread through the function, the array, the JSX, the object it belongs to. The ideal: the 💰 **is** the line(s) of code, commented out in place, so the learner can essentially just uncomment them top-to-bottom (and delete a stub) and have it work. Co-located 💰s turn an exercise into a series of small, obvious moves instead of a spec to decode.
