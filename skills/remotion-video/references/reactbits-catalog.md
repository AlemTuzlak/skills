...
## When to reach for reactbits vs. the default gradient variants

The default `<SceneBackground>` variants (`primary-glow`, `vignette`, `diagonal`, `flat`) are still the right choice for many scenes. **Pick a reactbits background only when one of the following is true**:
...
3. The product's metaphor maps directly to a reactbits effect (an audio product → `LineWaves` / `Waves`; a sync product → `LiquidEther` / `Threads`; a deploy product → `Hyperspeed` / `LightRays`).

**Stay on gradients when:**
- The scene is code-dense (delivery / `LibrarySwap` / `CodeSnippet`) — a busy background fights the code.
- The hook or CTA is text-only and benefits from a clean focus surface — a primary-glow gradient lets the headline carry.
- Render budget is tight (multi-format renders, low-power machine).
...
npx shadcn@latest add @react-bits/<Name>-TS-CSS
```

Use the **TS-CSS** variant in this Remotion project (the project is TypeScript; matching language avoids `// @ts-ignore` noise on typed scenes). The `JS-CSS`, `JS-TW`, `TS-TW` variants exist for non-typed or Tailwind projects — not used here.

The component lands at `src/components/<Name>/<Name>.tsx` (or wherever your `components.json` aliases resolve). The skill imports it from there into `<SceneBackground variant="reactbits">`.

## The 41 backgrounds

Columns:

- **Tech** — render path. **OGL-shader** = OGL renderer + custom GLSL fragment shader, drives `uTime` from a wall-clock rAF loop (needs adapter). **R3F** = `@react-three/fiber` + `useFrame` (needs adapter). **Three** = raw `three.js` with custom render loop. **Canvas2D** = HTML canvas, no WebGL. **CSS** = pure CSS keyframes (works as-is, animation-delay trick).
- **Mood** — primary narrative register the background sells. Pick from `calm`, `technical`, `hype`, `tension`, `playful`, `atmospheric`, `premium`.
- **Mouse default** — `on` means the upstream component reacts to cursor position by default; `off` means it doesn't. **All `on` backgrounds must have mouse interaction disabled** for video render (cursor doesn't exist in a recorded mp4).
- **Adapter** — `trivial` (≤10 lines: replace rAF with frame-driven `useEffect`), `moderate` (need to thread frame through R3F or canvas2D loop), `hard` (custom physics or postprocessing pipeline that resists frame-locking — use sparingly and consider skipping).
...
1. **Single-component, two prop snapshots.** Most reactbits backgrounds expose a small set of props (color, intensity, speed, amplitude). Render the same background in two states — calm/desaturated in the problem scene, saturated/intense in the solution — by swapping props per scene. Example: `Aurora` with `colorStops={brand.muted}` in problem → `colorStops={brand.primary}` in solution.
2. **Two complementary backgrounds across the arc.** When a single component can't carry the state-change, pair two from the same mood column. Example: `DarkVeil` (problem) → `LightRays` (solution) — both atmospheric, opposite intensity.

A reactbits background that holds the same props across the whole video is not pulling its weight. Either drive at least one prop from `frame` (or per-scene), or stay on the gradient variants.

## Frame-locking — the must-do adapter

**This is the load-bearing piece.** Reactbits backgrounds animate via `requestAnimationFrame` + `performance.now()`. Remotion renders by stepping a headless Chromium frame-by-frame, freezing wall-clock time. Dropped in naively, every captured frame would show the same `uTime` — the background is **frozen** in the rendered mp4 even though it animates fine in Studio preview.

Every reactbits background must be patched to drive its time source from `useCurrentFrame()` instead of `performance.now()`. The recipe lives in `src/reactbits-adapter.tsx` (scaffolded by the skill into the project). See **Phase 4.5 — Reactbits adapter** in the main SKILL.md for the workflow.

The short version (per OGL-shader component):

```tsx
// Inside the copied <Aurora>.tsx, in the useEffect that owns the renderer:
...
// And: delete the original `requestAnimationFrame(update)` loop.
// And: any `mouseInteraction` / `mouseReact` prop → false.
```

For R3F components (`Beams`, `Dither`): same pattern, but inside the parent component pass `frame` as a prop and use it directly in JSX attributes — never `useFrame` (R3F's clock is wall-driven; Remotion's `<ThreeCanvas>` overrides `frameloop="never"` but `useFrame` is still wrong for video).

For `hard` adapter components (`Ballpit`, `Hyperspeed`): consider whether a simpler component reaches the same mood. If `Hyperspeed` truly is the only fit (CTA "ship faster" finale), accept the render-cost hit and patch carefully — physics integration step must be `dt = 1/fps` (constant), not `dt = elapsed - lastElapsed`.

## Render config (Remotion)

When any scene uses a reactbits OGL/R3F background, the render command needs WebGL in the headless context:
...
`--gl=angle` is the right default for Windows + reactbits. On macOS/Linux `--gl=swangle` is more portable but slower. The skill defaults to `angle` and falls back to `swangle` only if the user reports black-frame output.

Long renders (≥60s composition) with shader backgrounds can hit ANGLE memory leaks. Either split renders into chunks (`--frames=0-450,451-900,...`) or stick to the trivial-adapter components for long videos.

## Safety: WCAG 2.3.1 photosensitive-seizure

Backgrounds with strobe/flash characteristics must respect the **three-flashes-per-second** ceiling (WCAG 2.3.1 / W3C Three Flashes). Specifically:

- `Lightning` — cap `intensity` to ≤0.6 and `frequency` to ≤2 strikes/sec. Default props ship higher.
- `LetterGlitch` — cap `glitchSpeed` so visible character cycling stays ≤3/sec on any fixed glyph cell.
- `PrismaticBurst` — cap `flashRate` ≤2/sec. The default tends to pulse fast.
- `FaultyTerminal` — flicker is fine; the strobe risk is from rapid scanline jumps. Cap `scanlineSpeed`.
...
- The **signature motif** rule (Phase 3.0) is still load-bearing. A reactbits background does not exempt the video from having a recurring custom visual element. The motif is what makes the video memorable; the background is the room it lives in.
- The **state-change** rule (Phase 3.4) still applies. If the chosen background looks identical across all scenes, it's decoration, not narrative.
...
- The **gradient variants are still the default**. This catalog is opt-in atmosphere, used per scene where it serves the story.
