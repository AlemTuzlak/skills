...
Implementation: use the `data-bg` variants in `styles.css` as the color-script surface. `primary-glow` reads as warm+on-brand, `vignette` reads as cool/pressing, `diagonal` reads as transitional.
