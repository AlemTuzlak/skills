...
export function keepRangesFromV3(edl) {
  const [num, den] = String(edl.timebase || "30/1").split("/").map(Number);
...
function main() {
  ...
  for (const bin of ["auto-editor", "ffmpeg", "ffprobe"]) {
    ...
    if (r.error && r.error.code === "ENOENT") { console.error(`[desilence] ${bin} is REQUIRED but not found on PATH.`); process.exit(1); }
  }

  const before = duration(o.input);
  const tmp = path.join(os.tmpdir(), `desilence-${process.pid}`);
  const v3 = `${tmp}.v3`, filterFile = `${tmp}.filter`;
  try {
    // 1) DETECT silence only (no encode) -> v3 timeline of kept segments
    const det = sh("auto-editor", [o.input, "--margin", o.margin, "--export", "v3", "-o", v3, "--no-open", "--progress", "none"]);
    if (det.status !== 0 || !existsSync(v3)) { console.error("[desilence] auto-editor detection failed:", det.stderr || det.stdout); process.exit(1); }
    const keep = keepRangesFromV3(JSON.parse(readFileSync(v3, "utf8")));
    if (keep.length === 0) { console.error("[desilence] no loud segments detected (margin too tight?)."); process.exit(1); }
    ...
    const r = sh("ffmpeg", [
      ...
      "-c:a", "aac", "-b:a", "256k", o.out,
    ]);
    if (r.status !== 0 || !existsSync(o.out)) { console.error("[desilence] ffmpeg cut failed:", r.stderr); process.exit(1); }
  } finally {
    for (const f of [v3, filterFile]) { try { rmSync(f, { force: true }); } catch { /* best-effort */ } }
  }

  const after = duration(o.out);
  process.stdout.write(JSON.stringify({
    ok: true, out: o.out, crf: o.crf,
    beforeSec: before, afterSec: after,
