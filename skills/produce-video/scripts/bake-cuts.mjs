...
export function keepsFromCuts(cuts, total) {
  ...
  const keeps = [];
  let cursor = 0;
  for (const [s, e] of sorted) { if (s > cursor) keeps.push([cursor, s]); cursor = Math.max(cursor, e); }
...
function main() {
  ...
  if (!o.input || !existsSync(o.input)) { console.error("[bake-cuts] input not found"); process.exit(1); }
  if (!o.out) { console.error("[bake-cuts] --out <file> required"); process.exit(1); }
  if (o.cutsJson) {
    try { o.cuts = JSON.parse(readFileSync(o.cutsJson, "utf8")); }
    catch (e) { console.error("[bake-cuts] could not read --cuts-json:", e.message); process.exit(1); }
  }
  if (!Array.isArray(o.cuts) || o.cuts.length === 0) { console.error("[bake-cuts] provide at least one --cut s,e (or --cuts-json)"); process.exit(1); }
  for (const c of o.cuts) {
    if (!Array.isArray(c) || c.length !== 2 || !Number.isFinite(c[0]) || !Number.isFinite(c[1]) || c[1] <= c[0]) {
      console.error("[bake-cuts] bad cut range (need start,end with end>start):", JSON.stringify(c)); process.exit(1);
    }
  }
  const total = o.total ?? probeDuration(o.input);
  if (total == null) { console.error("[bake-cuts] could not determine duration; pass --total <sec>"); process.exit(1); }

  const keeps = keepsFromCuts(o.cuts, total);
  if (keeps.length === 0) { console.error("[bake-cuts] cuts remove the entire clip"); process.exit(1); }
  ...
  parts.push(`${keeps.map((_, k) => `[v${k}][a${k}]`).join("")}concat=n=${keeps.length}:v=1:a=1[v][a]`);
  ...
  try {
    ...
    try { rmSync(filterFile, { force: true }); } catch { /* best-effort */ }
  }
  const removed = o.cuts.reduce((a, [s, e]) => a + (e - s), 0);
  process.stdout.write(JSON.stringify({ ok: true, out: o.out, keeps: keeps.length, removedSec: Math.round(removed * 100) / 100, finalSec: probeDuration(o.out) }) + "\n");
}
